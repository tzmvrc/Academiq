...
// Map subject name to Lucide icon
const getIconForSubject = (name: string) => {
  ...
  if (lowerName.includes("busin") || lowerName.includes("econ")) return Briefcase;
  ...
  if (lowerName.includes("lang") || lowerName.includes("literature")) return Languages;
  if (lowerName.includes("geog")) return Globe;
  if (lowerName.includes("sociol") || lowerName.includes("anthro")) return Users;
...
const Interests = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch top 15 popular tags
        const tagsRes = await axiosInstance.get("/tags?sort=popular&limit=15");
  ...
  // Fetch featured forums from top trending subjects
  useEffect(() => {
    const fetchFeaturedForums = async () => {
      ...
      try {
        const promises = topSubjects.map(subject =>
          axiosInstance.get(`/forums?subjectId=${subject.id}&limit=2&sort=upvotes`)
        );
        const results = await Promise.all(promises);
        const forums = results.flatMap(res => res.data.forums || []);
        const unique = forums.filter((f, idx, self) => self.findIndex(f2 => f2.id === f.id) === idx);
  ...
  const toggleFollow = async (subjectId: string, name: string, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    setSaving(true);
    try {
      await axiosInstance.post("/subjects/my-subjects", { subjectIds: Array.from(newSet) });
      toast({ title: isFollowing ? `Unfollowed ${name}` : `Following ${name}` });
    } catch (err) {
      console.error("Error saving followed subjects:", err);
  ...
  const topTrendingSubjects = [...subjects]
    .sort((a, b) => b.usage_count - a.usage_count)
    .slice(0, 6); // top 6 trending subjects as cards

  const followedSubjects = subjects.filter(s => followedSubjectIds.has(s.id));
  const visibleFollowed = showAllFollowed ? followedSubjects : followedSubjects.slice(0, 3);

  const discoverSubjects = subjects.filter(s => !followedSubjectIds.has(s.id));
  const filteredDiscover = discoverSubjects.filter(subject =>
    subject.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
  const visibleDiscover = showAllDiscover ? filteredDiscover : filteredDiscover.slice(0, 6);
  const hasMoreDiscover = filteredDiscover.length > 6;

  if (loading) {
    return (
      <div className="mx-auto max-w-4xl px-4 sm:px-6 py-6 sm:py-8">
        <h1 className="text-xl sm:text-2xl font-heading font-bold text-foreground mb-2">Your Interests</h1>
        <p className="text-sm sm:text-base text-muted-foreground mb-6 sm:mb-8">Loading topics...</p>
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-6 sm:py-8">
      ...
      {trendingTags.length > 0 && (
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-base sm:text-lg font-heading font-semibold text-foreground">Trending Tags</h2>
          </div>
          <div className="flex flex-wrap gap-2">
            {trendingTags.map((tag) => (
              ...
              >
                ...
                {tag.name}
                <span className="ml-1 text-xs text-primary/70">{tag.usage_count}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {/* Trending Subjects (rectangle cards) */}
      {topTrendingSubjects.length > 0 && (
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-4">
            <TrendingUp className="h-5 w-5 text-primary" />
            <h2 className="text-base sm:text-lg font-heading font-semibold text-foreground">Trending Subjects</h2>
          </div>
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {subject.usage_count} discussions
                      </p>
                    </div>
                  </div>
                  <button
      ...
      {/* Your Followed Subjects (with View More) */}
      ...
      {/* Discover More Subjects with search */}
      {discoverSubjects.length > 0 && (
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            {/* Show browse all only when not searching and there are more than 6 total discoverable subjects */}
          ...
          <div className="mb-4">
            <input
              ...
              onChange={(e) => {
                ...
                setShowAllDiscover(false); // reset expand when searching
              }}
          ...
          {/* Show "Show all X results" when searching and there are more than 6 matches */}
